...
import {
  BaseDTO, ColorantDTO, PrefillComponentDTO,
  ProductBaseCanDTO, ProductBaseDTO,
  ProductDTO
} from "../../models/colorant.model";
import {ColorFilter} from "../colorant/colorant.service";
...
function generateBaseProduct(productBaseCanId : number, base : BaseDTO, product : ProductDTO, can: number, unit : string, pricePerCan: number, percentage : number, barCode: string) : ProductBaseCanDTO {
  return {
...
const filterProductBase = function (productCode : string, baseType : string) {
  return function (item: ProductBaseCanDTO): boolean {
...
export class ProductBaseService {
  ...
  filterByProductCodeAndBaseType(productCode: string | null, baseType : string | null) : ProductBaseCanDTO[] | null{
    if(productCode == null || productCode == "" || baseType == null || baseType == ""){
      ...
      return this.listDBItems.filter(filterProductBase(productCode, baseType));
    }
  }
}
